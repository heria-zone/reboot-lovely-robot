 ...
 * <p>
 * <b>Config Location:</b> config/lovely_robot.properties in game directory. Created
...
public class TributeConfigs {
    ...
    private static final String CONFIG_FILE_NAME = Tribute.MODID;
    ...
    public static void register() {
        ...
        Tribute.LOGGER.info("Configuration loaded successfully");
    } // register()
    ...
    private static void buildConfigProvider(ConfigProvider provider) {
        ...
        for (String variant : LovelyConstant.TRIBUTE_VARIANTS) {
    ...
    /**
     * Loads dynamic entity configurations from config file into TributeConfigs HashMap.
     * <p>
    ...
    private static void loadDynamicEntityConfigs() {
        net.heriazone.lovelylib.source.tribute.TributeConfigs.Entities = new HashMap<>();
        //TributeConfigs.Entities.clear();
        ...
        for (String variant : LovelyConstant.TRIBUTE_VARIANTS) {
            ...
            try {
                ...
                SharedConfigs.EntityConfigData entityConfig = new SharedConfigs.EntityConfigData.Builder()
                        ...
                        .build();
                // Validate and store
                net.heriazone.lovelylib.source.tribute.TributeConfigs.Entities.put(variant, entityConfig.validateOrDefault());
            } catch (Exception e) {
                Tribute.LOGGER.warn("Failed to load config for variant '{}', using defaults: {}", variant, e.getMessage());
                net.heriazone.lovelylib.source.tribute.TributeConfigs.Entities.put(variant, getDefaultEntityConfig(variant));
            }
        }
        
        Tribute.LOGGER.info("Loaded dynamic entity configurations for {} variants", net.heriazone.lovelylib.source.tribute.TributeConfigs.Entities.size());
    } // loadDynamicEntityConfigs()
    ...
    private static SharedConfigs.EntityConfigData getDefaultEntityConfig(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant);
    } // getDefaultEntityConfig()

    // -- Default Value Getters --

    private static int getDefaultMaxLevel(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).maxLevel;
    } // getDefaultMaxLevel()

    private static float getDefaultAttackSpeed(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).attackSpeed;
    } // getDefaultAttackSpeed()

    private static float getDefaultMovementSpeed(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).movementSpeed;
    } // getDefaultMovementSpeed()

    private static float getDefaultBaseToughness(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).baseToughness;
    } // getDefaultBaseToughness()

    private static int getDefaultBaseHp(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).baseHp;
    } // getDefaultBaseHp()

    private static int getDefaultBaseAttack(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).baseAttack;
    } // getDefaultBaseAttack()

    private static int getDefaultBaseDefense(String variant) {
        return net.heriazone.lovelylib.source.tribute.TributeConfigs.getDefaultConfig(variant).baseDefense;
    ...
    public static void reload() {
        Tribute.LOGGER.info("Starting configuration reload from disk...");
        ...
        try {
            ...
            if (newConfig.isBroken()) {
                Tribute.LOGGER.error("Config file is broken or failed to parse, preserving existing configuration values");
                return;
            }
            ...
            Tribute.LOGGER.info("Configuration reload completed successfully");
        } catch (Exception e) {
            // Restore previous config on any error
            config = previousConfig;
            Tribute.LOGGER.error("Failed to reload configuration due to unexpected error, preserving existing configuration values: " + e.getMessage(), e);
        }
    } // reload()
