...
/**
 * Configuration management for Reboot LovelyRobotEntity mod.
 * <p>
 ...
 * <p>
 * <b>Config Location:</b> config/rlovelyr.properties in game directory. Created
 ...
 */
public class RebootConfigs {

    // -- Constants --

    private static final String CONFIG_FILE_NAME = Reboot.MODID;
    ...
    public static void register() {
        ...
        Reboot.LOGGER.info("Configuration loaded successfully");
    } // register()
    ...
    /**
     * Loads dynamic entity configurations from config file into RebootConfigs HashMap.
     * <p>
        ...
        for (String variant : LovelyConstant.REBOOT_VARIANTS) {
            ...
            try {
                ...
                SharedConfigs.EntityConfigData entityConfig = new SharedConfigs.EntityConfigData.Builder()
                        ...
                        .build();
                // Validate and store
                net.heriazone.lovelylib.source.reboot.RebootConfigs.Entities.put(variant, entityConfig.validateOrDefault());
            } catch (Exception e) {
                Reboot.LOGGER.warn("Failed to load config for variant '{}', using defaults: {}", variant, e.getMessage());
                net.heriazone.lovelylib.source.reboot.RebootConfigs.Entities.put(variant, getDefaultEntityConfig(variant));
            }
        }
        
        Reboot.LOGGER.info("Loaded dynamic entity configurations for {} variants", net.heriazone.lovelylib.source.reboot.RebootConfigs.Entities.size());
    } // loadDynamicEntityConfigs()
    ...
    private static SharedConfigs.EntityConfigData getDefaultEntityConfig(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant);
    } // getDefaultEntityConfig()

    // -- Default Value Getters --

    private static int getDefaultMaxLevel(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).maxLevel;
    } // getDefaultMaxLevel()

    private static float getDefaultAttackSpeed(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).attackSpeed;
    } // getDefaultAttackSpeed()

    private static float getDefaultMovementSpeed(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).movementSpeed;
    } // getDefaultMovementSpeed()

    private static float getDefaultBaseToughness(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).baseToughness;
    } // getDefaultBaseToughness()

    private static int getDefaultBaseHp(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).baseHp;
    } // getDefaultBaseHp()

    private static int getDefaultBaseAttack(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).baseAttack;
    } // getDefaultBaseAttack()

    private static int getDefaultBaseDefense(String variant) {
        return net.heriazone.lovelylib.source.reboot.RebootConfigs.getDefaultConfig(variant).baseDefense;
    ...
    public static void reload() {
        Reboot.LOGGER.info("Starting configuration reload from disk...");
        ...
        try {
            ...
            if (newConfig.isBroken()) {
                Reboot.LOGGER.error("Config file is broken or failed to parse, preserving existing configuration values");
                return;
            }
            ...
            Reboot.LOGGER.info("Configuration reload completed successfully");
        } catch (Exception e) {
            // Restore previous config on any error
            config = previousConfig;
            Reboot.LOGGER.error("Failed to reload configuration due to unexpected error, preserving existing configuration values: " + e.getMessage(), e);
        }
    } // reload()
