...
/**
 * <p>Enhanced model variant feature with registry integration and entity-specific filtering.<p>
 * <p>
 * <b>Architecture:</b> Bridges entity-specific model configuration with global variant registry,
 ...
 */
public class ModelVariantFeature implements IVariantFeature<IModelVariant> {

    // -- Fields --

    private final Map<String, Set<String>> entityVariants;
    private final Map<String, String> defaultVariants;
    private final ResourceMap<String, ResourceLocation> additionalModels;

    // -- Constructor --

    /**
     * Creates empty model variant feature with no configured variants.
     * <p>
     ...
     */
    public ModelVariantFeature() {
        this.entityVariants = new HashMap<>();
        this.defaultVariants = new HashMap<>();
        this.additionalModels = new ResourceMap<>();
    } // Constructor: ModelVariantFeature ()
    ...
    @Override
    public Collection<IModelVariant> getAvailableVariants(String entityKey) {
        ...
        return enabledVariants.stream()
                .map(variantKey -> VariantRegistries.MODELS.get(variantKey))
    ...
    @Override
    public IModelVariant getDefaultVariant(String entityKey) {
        String defaultKey = defaultVariants.get(entityKey);
        if (defaultKey != null) {
            Optional<IModelVariant> defaultVariant = VariantRegistries.MODELS.get(defaultKey);
        ...
        return getAvailableVariants(entityKey).stream()
                .max(Comparator.comparingInt(IModelVariant::getPriority))
                .orElse(null);
    } // getDefaultVariant ()

    @Override
    public IModelVariant getRandomVariant(String entityKey) {
        Collection<IModelVariant> available = getAvailableVariants(entityKey);
        if (available.isEmpty()) {
            return null;
        }

        List<IModelVariant> variantList = new ArrayList<>(available);
    ...
    public boolean hasVariant(String entityKey, String variantKey) {
        ...
        Optional<IModelVariant> variant = VariantRegistries.MODELS.get(variantKey);
     ...
     */
    public ModelVariantFeature withVariant(String entityKey, String variantKey) {
     ...
     * @return this feature instance for chaining
     */
    public ModelVariantFeature withDefault(String entityKey, String variantKey) {
        if (entityKey != null && variantKey != null) {
     ...
     * @return this feature instance for chaining
     */
    public ModelVariantFeature withVariants(String entityKey, String... variantKeys) {
    ...
    /**
     * Associates model identifier with specified variant name.
     * <p>
     * <b>Legacy Support:</b> Maintains compatibility with direct model registration.
     * Consider using registry-based variants for new implementations.
     * <p>
     * <b>Fluent API:</b> Returns this instance for method chaining.
     *
     * @param variant variant name (e.g., "body_1", "seasonal_winter")
     * @param model model identifier to associate with variant
     * @return this feature instance for chaining
     */
    public ModelVariantFeature withModel(String variant, ResourceLocation model) {
        if (variant != null && model != null) {
            this.additionalModels.put(variant, model);
        }
        return this;
    } // withModel ()
     ...
     * <b>Fluent API:</b> Returns this instance for method chaining.
     *
     * @param basePath base path for model identifiers (e.g., "entity/robot")
    ...
    public ModelVariantFeature withBodyVariants(String basePath, String... variants) {
        ...
        for (int i = 0; i < variants.length; i++) {
            if (variants[i] != null) {
                ...
                String modelPath = basePath + "/" + variants[i];
                ResourceLocation modelId = ResourceLocation.parse(modelPath);
                this.additionalModels.put(variantKey, modelId);
            }
        }
    ...
    // -- Legacy Model Queries --

    /**
     * Returns model identifier for specified variant.
     * <p>
     * <b>Legacy Support:</b> Checks additional models first, then registry variants.
     * <p>
     * <b>Fallback Behavior:</b> Returns null if variant not configured.
     * Callers should handle null or provide default model as fallback.
     *
     * @param variant variant name to retrieve
     * @return model identifier for variant, or null if not configured
     */
    public ResourceLocation getModel(String variant) {
        // Check additional models first (legacy support)
        ResourceLocation additionalModel = this.additionalModels.get(variant);
        if (additionalModel != null) {
            return additionalModel;
        }

        // Check registry variants (new system)
        Optional<IModelVariant> registryVariant = VariantRegistries.MODELS.get(variant);
    ...
    /**
     * Checks if model is configured for specified variant.
     *
     * @param variant variant name to check
     * @return true if model is configured for variant, false otherwise
     */
    public boolean hasModel(String variant) {
        return this.additionalModels.has(variant) || VariantRegistries.MODELS.contains(variant);
    } // hasModel ()

    /**
     * Returns randomly selected model identifier from configured variants.
     * <p>
     * <b>Legacy Support:</b> Selects from additional models only.
     * <p>
     * <b>Performance:</b> O(1) after initial cache build due to ResourceMap caching.
     *
     * @return randomly selected model identifier, or null if no variants configured
     */
    public ResourceLocation getRandomModel() {
        return this.additionalModels.getRandom();
    } // getRandomModel ()
