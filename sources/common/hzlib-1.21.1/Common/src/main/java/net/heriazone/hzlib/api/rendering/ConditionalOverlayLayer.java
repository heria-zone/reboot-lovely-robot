 ...
 * <p>
 * <b>Design Decision:</b> Conditional overlay with state-based texture swapping.
...
public class ConditionalOverlayLayer<T extends NativeEntity> implements IInternalRenderLayer<T> {
    ...
    /**
     * Creates conditional overlay layer with default texture.
     *
     * @param defaultTexturePath texture path to render when no conditions match (idle state)
     */
    public ConditionalOverlayLayer(String defaultTexturePath) {
        this.defaultTexturePath = defaultTexturePath;
    } // Constructor: ConditionalOverlayLayer()
     ...
     */
    public ConditionalOverlayLayer<T> addConditionalTexture(Predicate<T> condition, String texturePath) {
    ...
    public boolean shouldRender(T entity, float partialTick) {
        return !defaultTexturePath.isEmpty() && !conditionalTextures.isEmpty();
    } // shouldRender()
