...
public class AiConditionalWanderGoal extends WaterAvoidingRandomStrollGoal {
    ...
    public boolean canUse() {
        ...
        if (ownerStillTicks < LovelyConfigs.OwnerStillThreshold) {
            robot.setCanWander(false);
            return false;
        }
        ...
        if (robot.getRandom().nextDouble() > LovelyConfigs.WanderChance) {
            robot.setCanWander(false);
            return false;
        }
        ...
        if (distanceToOwner > LovelyConfigs.WanderRadiusMax) {
            robot.setCanWander(false);
            return false;
        }
    ...
    public void start() {
        ...
        int durationRange = LovelyConfigs.WanderDurationMax - LovelyConfigs.WanderDurationMin;
        wanderDuration = LovelyConfigs.WanderDurationMin + robot.getRandom().nextInt(durationRange + 1);
    ...
    public void stop() {
        ...
        int cooldownRange = LovelyConfigs.WanderCooldownMax - LovelyConfigs.WanderCooldownMin;
        wanderCooldown = LovelyConfigs.WanderCooldownMin + robot.getRandom().nextInt(cooldownRange + 1);
        
        // Path back to wander start position
        robot.getNavigation().moveTo(wanderStartX, wanderStartY, wanderStartZ, LovelyConfigs.MovementFollowOwner);
