...
public class AiBaseDefenseGoal extends Goal {
    ...
    @Override
    public void tick() {
        double distanceToBase = this.entity.squaredDistanceTo(this.entity.getBaseX(), this.entity.getBaseY(), this.entity.getBaseZ());
    ...
    private void tickPatrol() {
            ...
            if (--lookTimer <= 0) {
                ...
                if (currentPatrolTarget != null) {
                    ...
                    entity.getLookControl().lookAt(
                        ...
                        (float) entity.getMaxLookPitchChange()
                    );
                } else {
        ...
        if (currentPatrolTarget == null || navigation.isIdle() || entity.squaredDistanceTo(currentPatrolTarget.getX(), currentPatrolTarget.getY(), currentPatrolTarget.getZ()) < 4.0) {
            ...
            if (currentPatrolTarget != null) {
                navigation.startMovingTo(currentPatrolTarget.getX(), currentPatrolTarget.getY(), currentPatrolTarget.getZ(), speed);
                
                // Set random pause duration for when we reach this point
                int pauseRange = LovelyConfigs.Common.PatrolPauseDurationMax - LovelyConfigs.Common.PatrolPauseDurationMin;
                patrolPauseTimer = LovelyConfigs.Common.PatrolPauseDurationMin + entity.getRandom().nextInt(pauseRange + 1);
    ...
    private void tickGuard() {
        ...
        double distanceToBase = entity.squaredDistanceTo(entity.getBaseX(), entity.getBaseY(), entity.getBaseZ());
    ...
    private void setRandomStateDuration() {
        ...
        switch (currentDefenseState) {
            case PATROL:
                durationMin = LovelyConfigs.Common.PatrolDurationMin;
                durationMax = LovelyConfigs.Common.PatrolDurationMax;
                break;
            case GUARD:
                durationMin = LovelyConfigs.Common.GuardDurationMin;
                durationMax = LovelyConfigs.Common.GuardDurationMax;
                break;
            default:
                durationMin = 600;
                durationMax = 900;
