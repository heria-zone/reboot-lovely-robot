...
public abstract class LovelyRobotEntity extends InternalEntity implements GeoEntity {
    ...
    protected void initGoals() {
        ...
        this.targetSelector.add(4, new AiAutoAttackGoal<>(this, MobEntity.class, LovelyConfigs.Common.AttackChance, true, false,
    ...
    protected void handleAttackTarget(@NotNull Entity target) {
        ...
        if(this.getCurrentLevel() < this.getMaxLevel() && !(target instanceof PlayerEntity) && !this.getWorld().isClient) {
    ...
    protected boolean handleDamage (DamageSource source, float amount) {
        ...
        if (this.getCurrentLevel() < this.getMaxLevel() && !(entity instanceof PlayerEntity) && entity instanceof LivingEntity && !this.getWorld().isClient) {
     ...
     * @return attribute container with configured values
     */
    public static DefaultAttributeContainer createAttributes(NativeEntityType entity) {
        return AnimalEntity.createMobAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, entity.getData().getMaxHealth())
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, entity.getData().getAttackDamage())
                .add(EntityAttributes.GENERIC_ATTACK_SPEED, entity.getData().getAttackSpeed())
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, entity.getData().getMoveSpeed())
                .add(EntityAttributes.GENERIC_ARMOR, entity.getData().getArmor())
                .add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, entity.getData().getArmorToughness())
                .build();
    } // createAttributes ()
