 ...
 * <p>
 * <b>Design Decision:</b> Server directory as key rather than ServerWorld instance
...
public class RobotRegistryManager {
     ...
     */
    @NotNull
    public static OwnerRobotRegistry getRegistry(@NotNull ServerWorld level) {
        Objects.requireNonNull(level, "ServerWorld cannot be null");
     ...
     * <p>
     * <b>Use Case:</b> Convenience method for clearing registry using ServerWorld.
     * 
     * @param level server level to clear registry for (must not be null)
     * @throws NullPointerException if level is null
     */
    public static void clearRegistry(@NotNull ServerWorld level) {
        Objects.requireNonNull(level, "ServerWorld cannot be null");
     ...
     */
    @NotNull
    private static String getServerId(@NotNull ServerWorld level) {
        try {
            // Get server directory path as unique identifier
            File serverDir = level.getServer().getRunDirectory();
