 ...
 * <p>
 * <b>Architecture:</b> Extends vanilla WanderAroundFarGoal with sophisticated
 ...
 */
public class AiConditionalWanderGoal extends WanderAroundFarGoal {
    ...
    public boolean canStart() {
        ...
        if (ownerStillTicks < LovelyConfigs.Common.OwnerStillThreshold) {
            robot.setCanWander(false);
            return false;
        }
        ...
        if (robot.getRandom().nextDouble() > LovelyConfigs.Common.WanderChance) {
            robot.setCanWander(false);
            return false;
        }
        ...
        if (distanceToOwner > LovelyConfigs.Common.WanderRadiusMax) {
            robot.setCanWander(false);
            return false;
        }
    ...
    public void start() {
        ...
        wanderStartYaw = robot.getYaw();
        wanderStartPitch = robot.getPitch();
        
        // Set random wander duration (5-10 seconds by default)
        int durationRange = LovelyConfigs.Common.WanderDurationMax - LovelyConfigs.Common.WanderDurationMin;
        wanderDuration = LovelyConfigs.Common.WanderDurationMin + robot.getRandom().nextInt(durationRange + 1);
    ...
    public void stop() {
        ...
        int cooldownRange = LovelyConfigs.Common.WanderCooldownMax - LovelyConfigs.Common.WanderCooldownMin;
        wanderCooldown = LovelyConfigs.Common.WanderCooldownMin + robot.getRandom().nextInt(cooldownRange + 1);
        
        // Path back to wander start position
        robot.getNavigation().startMovingTo(wanderStartX, wanderStartY, wanderStartZ, LovelyConfigs.Common.MovementFollowOwner);
        
        // Set rotation to original facing direction
        robot.setYaw(wanderStartYaw);
        robot.setPitch(wanderStartPitch);
    } // stop
