...
public class AiFollowOwnerGoal extends FollowOwnerGoal {
     ...
     *
     * @param tameable robot entity (cast to TamableAnimal for parent compatibility)
     ...
     */
    public AiFollowOwnerGoal(LovelyRobotEntity tameable, double speed, float minDistance, float maxDistance) {
    ...
    public boolean canUse() {
        ...
        double velocitySquared = owner.getDeltaMovement().lengthSqr();
        ...
        if (inCombat) {
            ...
            if (distance <= SharedConfigs.Common.FollowDistanceMax) return false;
        } else {
            // Not in combat: Follow to maintain minimum distance (2 blocks)
            // This keeps robot close to owner at all times
            if (distance <= SharedConfigs.Common.FollowDistanceMin) return false;
        }
