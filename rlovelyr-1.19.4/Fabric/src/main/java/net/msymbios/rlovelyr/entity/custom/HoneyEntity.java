...
public class HoneyEntity extends TameableEntity implements GeoEntity {

    // -- Variables --
    private static final TrackedData<String> VARIANT = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.STRING);
    private static final TrackedData<Integer> TEXTURE_ID = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);

    private static final TrackedData<Integer> STATE = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Boolean> AUTO_ATTACK = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.BOOLEAN);

    private static final TrackedData<Integer> MAX_LEVEL = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> LEVEL = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> EXP = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);

    private static final TrackedData<Integer> FIRE_PROTECTION = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> FALL_PROTECTION = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> BLAST_PROTECTION = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> PROJECTILE_PROTECTION = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.INTEGER);

    private static final TrackedData<Float> BASE_X = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> BASE_Y = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> BASE_Z = DataTracker.registerData(HoneyEntity.class, TrackedDataHandlerRegistry.FLOAT);;
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return MobEntity.createMobAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.MAX_HEALTH))
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.ATTACK_DAMAGE))
                .add(EntityAttributes.GENERIC_ATTACK_SPEED, ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.ATTACK_SPEED))
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.MOVEMENT_SPEED))
                .add(EntityAttributes.GENERIC_ARMOR, ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.ARMOR))
                .add(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.ARMOR_TOUGHNESS));
    } // setAttributes ()
    ...
    public int getMaxLevel(){
        var value = (int)ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.MAX_LEVEL);
        var oldValue = value;
    ...
    public int getLevel(){
        var level = (int)(ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.MAX_LEVEL));
    ...
    public int getAttackValue() {
        var attack = ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.ATTACK_DAMAGE);
        return (int)(attack + this.getLevel() * attack / 50);
    } // getAttackValue ()

    public int getDefenseValue() {
        var defense = ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.DEFENSE);
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable NbtCompound entityNbt) {
        ...
        this.setMaxLevel((int)ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.MAX_LEVEL));
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if ("fire".equals(source.getName()) && amount >= 1.0f && this.getFireProtection() > 0)
            amount *= (100.0f - this.getFireProtection()) / 100.0f;

        if ("fall".equals(source.getName()) && amount >= 1.0f && this.getFallProtection() > 0)
            amount *= (100.0f - this.getFallProtection()) / 100.0f;

        if ("explosion".equals(source.getName()) && amount >= 1.0f && this.getBlastProtection() > 0)
            amount *= (100.0f - this.getBlastProtection()) / 100.0f;

        if ("projectile".equals(source.getName()) && amount >= 1.0f && this.getProjectileProtection() > 0)
            amount *= (100.0f - this.getProjectileProtection()) / 100.0f;

        if (amount < 1.0f) return false;

        if(!world.isClient) {
            if("fire".equals(source.getName()) && canLevelUpFireProtection()) this.setFireProtection(this.getFireProtection() + 1);
            if("fall".equals(source.getName()) && canLevelUpFallProtection()) this.setFallProtection(this.getFallProtection() + 1);
            if("explosion".equals(source.getName()) && canLevelUpBlastProtection()) this.setBlastProtection(this.getBlastProtection() + 1);
            if("projectile".equals(source.getName()) && canLevelUpProjectileProtection()) this.setProjectileProtection(this.getProjectileProtection() + 1);
        }
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(VARIANT, RobotVariant.Honey.getName());
        this.dataTracker.startTracking(TEXTURE_ID, 0);

        this.dataTracker.startTracking(STATE, 0);
        this.dataTracker.startTracking(AUTO_ATTACK, false);

        this.dataTracker.startTracking(MAX_LEVEL, (int)ModMetrics.getAttributeValue(RobotVariant.Honey, RobotAttribute.MAX_LEVEL));
