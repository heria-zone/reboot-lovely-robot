...
public class HealthBarControl extends Control {
    ...
    public void onRenderTooltip(double mouseX, double mouseY, float partialTicks) {
        List<Text> tooltip = StatsScreen.createModifiableFloatAttributeTooltip(blockling.getStats().maxHealth, Formatting.DARK_GREEN);
        tooltip.add(0, Text.literal(Formatting.GOLD + LovelyRobotID.getAttributeTranslation("health.name").getString()));

        renderTooltip(mouseX, mouseY, (Text) tooltip.stream().map(Text::getContent).collect(Collectors.toList()));
    }
