...
public abstract class RobotEntity extends InternalEntity implements GeoEntity {
    ...
    public ItemStack getItemBySlot(EquipmentSlot slot) {
        switch (slot.getType()){
            case HAND: {
                ...
                if(lootingLevel > 0) {
                    //tempSword.enchant(Enchantments, lootingLevel);
                }
                return tempSword;
            }
            default: {
    ...
    protected void handleItemDrop() {
        ...
        CompoundTag nbt = Objects.requireNonNull(dropItem.get(DataComponents.CUSTOM_DATA)).copyTag();
        ...
        dropItem.set(DataComponents.CUSTOM_DATA, CustomData.of(nbt));

        //if (!customName.isEmpty()) dropItem.set(Component.nullToEmpty(customName).copy().append(Utility.getRandomTitle()).withStyle(ChatFormatting.DARK_PURPLE));
